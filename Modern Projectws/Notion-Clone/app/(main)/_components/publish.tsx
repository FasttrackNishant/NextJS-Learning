                ...
                {initialData.isPublished ? (
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                            <input value={url} className="flex-1 px-1 text-xs border rounded-l-md h-8 bg-muted truncate" disabled />
                            <Button onClick={onCopy} disabled={copied} className="h-8 rounded-l-none">
                                {copied ? <Check className="h-4 w-4" /> : <Copy className="w-4 h-4" />}
                            </Button>
                        </div>
                        <Button onClick={onUnpublish} size="sm" className="w-full text-xs" disabled={isSubmitting}>
                            Unpublish
                        </Button>
                    </div>
                ) : (
